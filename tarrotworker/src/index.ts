			...
			const messages = [
				...
				},
				{
					...
					content: `Kartlar: ${cards
						.map(
							(card) =>
								`${card.Name} (${card.reversed ? 'Ters' : 'Düz'}) - ${card.Meaning} - ${card.Reversed} - ${card.Astrological} - ${
									card.Element
								} - ${card.Chakra}`
						)
						.join(', ')}
